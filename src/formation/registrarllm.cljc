...
(def default-corporate-intel-screen
  ...
  in `formation.corporate-intel/screen` (or an equivalent). Not required
  from this namespace directly -- keeping the dependency optional at the
  registrarllm level, injected only by whoever builds the advisor."
...
(defn- screen-kyc
  ...
  `screen-fn` (officer name -> corporate-intel result, see
  `formation.corporate-intel/screen`) is consulted ONLY once the local
  ...
  (let [off (store/officer db subject)]
    (cond
      ...
      :else
      (let [ci (screen-fn (:name off))]
        (cond
          (:hit? ci)
          {:summary    (str (:name off) ": corporate-intelligence 照会で制裁/PEPフラグを検出")
          ...
          {:summary    (str (:name off) ": corporate-intelligence 照会が人手レビュー待ち")
          ...
          (:held? ci)
          {:summary    (str (:name off) ": corporate-intelligence 照会が拒否された(契約/設定の問題)")
          ...
          :else
          {:summary    (str (:name off) ": 制裁リスト非一致、本人確認書類あり")
...
(defn mock-advisor
  ...
  opts:
    :corporate-intel-screen -- officer name -> corporate-intel result (see
      `formation.corporate-intel/screen`). Default: no-op (never changes a
