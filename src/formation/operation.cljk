(ns formation.operation
  "OperationActor -- one formation operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (Registrar-LLM) is
  sealed into a single node (:advise); its proposal is ALWAYS routed
  through the RegistrarGovernor (:govern) and the rollout phase gate
  ...
  One graph run = one formation operation (intake -> advise -> govern ->
  ...
  decision to a human operator (the licensed registered agent / customer
  success reviewer). The approver resumes with
  `{:approval {:status :approved}}` (or :rejected). `:filing/submit`
...
(defn build
  ...
  opts:
    :advisor      -- a `formation.registrarllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (registrarllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Registrar-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (registrarllm/-advise advisor store request)]
            {:proposal p :audit [(registrarllm/trace request p)]})))

      ;; RegistrarGovernor -- independent censor (separate system than the LLM).
      (g/add-node :govern
