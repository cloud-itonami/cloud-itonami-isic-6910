...
5チェック、優先順位順。最初の3つは HARD（人間が承認で上書き不可）:
...
4. **confidence floor** -- confidence が閾値未満なら escalate。
5. **actuation gate** -- `:stake :actuation`（実際の政府提出・実際の
   手数料送金）は常に escalate。**`formation.phase` のどのフェーズの
   `:auto` 集合にも `:filing/submit` を含めない**ことと合わせて、
...
- `phase_test.clj` -- 「`:filing/submit` はどのフェーズの `:auto` にも
  含まれない」構造不変条件。
